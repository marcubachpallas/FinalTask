...
void DebugSystem::updateimGUI_(float dt) {
	...
	if (show_imGUI_)
	{
		...
		ImGui::Text("Show scene materials");
		...
		// ******** SHAPE EDITOR *********
		ImGui::Begin("Porperties", &show_imGUI_, ImGuiWindowFlags_MenuBar);
		if (ImGui::TreeNode("BlendShapes")) {
			ImGui::Text("Select expression");
			if (ImGui::Button("Smily")) {
			...
			ImGui::Text("Slider face epxrecion");
		...
		if (ImGui::TreeNode("Particles Color")) {
